...
public class SimpleStatistics implements Statistics {
	...
	public double getProcessedMessageAverageTime() {
		return this.processMessageCount.get() == 0 ? 0 : (double) this.processMessageTotalTime.get() / this.processMessageCount.get();
	}
	...
	public double getRecvMessageCountPerSecond() {
		final long duration = this.stopTime == -1 ? System.currentTimeMillis() - this.startTime : this.stopTime - this.startTime;
