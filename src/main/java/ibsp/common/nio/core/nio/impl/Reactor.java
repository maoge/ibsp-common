...
import ibsp.common.nio.core.core.impl.AbstractSession;
...
public final class Reactor extends Thread {
	...
	public void run() {
		...
		while (this.selectorManager.isStarted() && this.selector.isOpen()) {
			try {
				...
				logger.error("Reactor select error:" + e.getMessage());
		...
		if (this.selector != null) {
			if (this.selector.isOpen()) {
				try {
					...
					logger.error("stop reactor error:{}", e.getMessage());
				}
			}
		}
	...
	private boolean lookJVMBug(final long before, final int selected, final long wait) throws IOException {
		 ...
		 */
		if (JVMBUG_THRESHHOLD > 0 && selected == 0 && wait > JVMBUG_THRESHHOLD && now - before < wait / 4 && !this.wakenUp.get() /*
																																 * waken
																																 * up
																																 */
			...
			if (this.jvmBug.get() >= JVMBUG_THRESHHOLD2) {
				...
				try {
					this.lastJVMBug = now;
					logger.error("JVM bug occured at " + new Date(this.lastJVMBug)
							+ ",http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6403933,reactIndex=" + this.reactorIndex);
					if (this.jvmBug1) {
						logger.error("seeing JVM BUG(s) - recreating selector,reactIndex=" + this.reactorIndex);
					} else {
						this.jvmBug1 = true;
						logger.error("seeing JVM BUG(s) - recreating selector,reactIndex=" + this.reactorIndex);
					}
				...
				if (this.jvmBug0) {
					logger.error("seeing JVM BUG(s) - cancelling interestOps==0,reactIndex=" + this.reactorIndex);
				} else {
					this.jvmBug0 = true;
					logger.error("seeing JVM BUG(s) - cancelling interestOps==0,reactIndex=" + this.reactorIndex);
				}
	...
	final void dispatchEvent(final Set<SelectionKey> selectedKeySet) {
		...
		while (it.hasNext()) {
			...
			try {
				...
				this.controller.closeSelectionKey(key);
				this.controller.notifyException(e);
				logger.error("Reactor dispatch events error:" + e.getMessage());
				if (this.selector.isOpen()) {
					continue;
				} else {
					break;
