...
import ibsp.common.nio.core.core.impl.FileWriteMessage;
import ibsp.common.nio.core.core.impl.FutureImpl;
import ibsp.common.nio.core.core.impl.PoisonWriteMessage;
...
public abstract class AbstractNioSession extends AbstractSession implements NioSession {
	...
	private static Logger logger = LoggerFactory.getLogger(AbstractNioSession.class);
	...
	protected final void flush0() {
		...
		try {
			...
			this.close0();
			this.onException(t);
			logger.error("Flush error:{}", t.getMessage());
		} finally {
			if (tmpKey != null) {
				// Cancel the key.
				tmpKey.cancel();
			...
			if (writeSelector != null) {
				try {
					...
					logger.error("Temp selector selectNow error:{}", e.getMessage());
				}
				// return selector
