...
import ibsp.common.nio.service.callback.SingleRequestCallBack;
...
public class DefaultConnection implements Connection {
	...
	private static Logger logger = LoggerFactory.getLogger(DefaultConnection.class);
	...
	private BooleanAckCommand createTimeoutCommand(final CommandHeader header, final InetSocketAddress address) {
		final BooleanAckCommand value = this.remotingContext.getCommandFactory().createBooleanAckCommand(header, ResponseStatus.TIMEOUT,
				"等待响应超时");
	...
	void dispose() {
		for (final Integer opaque : this.requestCallBackMap.keySet()) {
			...
			if (requestCallBack != null) {
				requestCallBack.onResponse(this.removeOpaqueToGroupMapping(opaque), this.createTimeoutCommand(new CommandHeader() {
	...
	public ResponseCommand invoke(final RequestCommand requestCommand, final long time, final TimeUnit timeUnit)
		...
		this.checkFlow();
		final SingleRequestCallBack requestCallBack = new SingleRequestCallBack(requestCommand.getRequestHeader(),
	...
	public void transferFrom(final IoBuffer head, final IoBuffer tail, final FileChannel channel, final long position, final long size,
			final Integer opaque, final SingleRequestCallBackListener listener, final long time, final TimeUnit unit)
